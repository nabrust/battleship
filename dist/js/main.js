...
var battleship = (function() {
  ...
  var flagShot = true; // флаг испоьзуется для проверки очереди выстрела
  ...
  var _setUpListners = function() {
    $('.battlefields__enemy-field').find('.table-cell').not('.hat').one('click', function () {
      if (flagShot === true) {
        fire($(this), 'player');
      }
    });
  ...
  var
    ...
    duration = 2000; // промежуток времени между выстрелами компьютера
    ...
    var controller = function() {
      if (initiator === 'player') {
        ...
        if (cell.children().is('.hit')) { // если было попадание
          ...
          setTimeout(computerLogic, duration);
        }
